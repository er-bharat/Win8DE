...
static int currentVolume() {
    ...
    auto m = re.match(proc.readAllStandardOutput());
...
static void changeVolume(int delta) {
    const QString arg = QString::number(std::abs(delta)) + "%" + (delta > 0 ? "+" : "-");
...
int main(int argc, char *argv[]) {
    ...
    if (!tryConnect()) {
        ...
        QProcess::startDetached("Win8OSD-server"); // replace with actual server path
        
        // Wait until server is ready, up to ~1 second
        int retries = 20;
        while (retries-- > 0 && !tryConnect()) {
            QThread::msleep(50);
        }
        
        if (!socket.isOpen()) {
            qCritical() << "Failed to connect to OSD server!";
            return 1;
        }
    }
    
    // Send OSD command
    const QString msg = QString("%1 %2 %3\n").arg(mode).arg(value).arg(muted ? 1 : 0);
